...
import id.indosw.digiflazz.api.commandvalue.CommandValue.CMD_PAY_BILLING
import id.indosw.digiflazz.api.controller.PayBillingRequestController
import id.indosw.digiflazz.api.sign.SignMaker.getSign

class RequestPayBilling(val activity: Activity) {
    ...
    fun startRequestPayBilling(requestListener: RequestListener?) {
        val signature = getSign(username!!, key!!, refId!!)
        PayBillingRequestController.instance!!.execute(this, backendUrl, username, key, CMD_PAY_BILLING, sku, custNumber, refId, signature, requestListener!!)
    }
